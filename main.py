...
@app.post('/test')
def createTest():
    img1 = cv2.imread(rootFolder + "girl\\girl-7-light-preview.png")
    img2 = cv2.imread(rootFolder + "girl\\girl-7-clothes.png", cv2.IMREAD_UNCHANGED)
    img3 = cv2.imread(rootFolder + "girl\\girl-7-hair-27.png", cv2.IMREAD_UNCHANGED)

    imgResult = cvzone.overlayPNG(img1, img2, [0, 0])
...
async def createImageRequest():
    ...
    img = np.array(Image.open("D:\\ME\\2022\\Python\\Folder_Image_Test\\girl\\girl-7-light-preview.png"))
    img_overlay_rgba = np.array(Image.open("D:\\ME\\2022\\Python\\Folder_Image_Test\\girl\\girl-7-clothes.png"))
    ...
    # x_offset = y_offset = 50
    #
    # l_img[y_offset:y_offset + s1_img.shape[0], x_offset:x_offset + s1_img.shape[1]] = s1_img
    #
    # y1, y2 = y_offset, y_offset + s1_img.shape[0]
    # x1, x2 = x_offset, x_offset + s1_img.shape[1]
    #
    # alpha_s = s1_img[:, :, 3] / 255.0
    # alpha_l = 1.0 - alpha_s
    #
    # for c in range(0, 3):
    #     l_img[y1:y2, x1:x2, c] = (alpha_s * s1_img[:, :, c] + alpha_l * l_img[y1:y2, x1:x2, c])
    ...
    Image.fromarray(img_result_Main).save("img_result_main.jpg")
    ...
    try:
        ...
        for i in range(0, data.numbers):
            ...
            back_im = remove_background(back_im)
